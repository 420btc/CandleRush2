...
export default function BetHistory() {
  ...
  return (
    <div className="h-full flex-1 min-h-0 w-full flex flex-col">
      <div className={`flex ${isMobile ? 'gap-1' : 'gap-2'} mb-1 justify-center`}>
        <button
          className={`${isMobile ? 'px-2 py-1 text-xs' : 'px-3 py-1 text-sm'} rounded-lg font-bold border-2 transition-all ${!showAchievements ? 'bg-yellow-400 text-black border-yellow-400' : 'bg-black/60 text-yellow-400 border-yellow-400 hover:bg-yellow-900/30'}`}
      ...
      {showAchievements ? (
        ...
        <ScrollArea className="h-full flex-1 min-h-0 w-full">
          <div className={`${isMobile ? 'space-y-1' : 'space-y-0'} pb-4`}>

...
export default function CandlestickChart({ candles, currentCandle, viewState, setViewState, verticalScale = 1, setVerticalScale, showVolumeProfile, setShowVolumeProfile, showCrossCircles, setShowCrossCircles }: CandlestickChartProps & { setVerticalScale?: (v: number) => void, showCrossCircles?: boolean, setShowCrossCircles?: (v: boolean | ((v: boolean) => boolean)) => void }) {
  ...
  const [selectedCandle, setSelectedCandle] = useState<{ timestamp: number; price: number } | null>(null);
  ...
  const drawChart = useCallback(() => {
    ...
    // --- Draw Selected Candle Line & Tooltip (Double Click) ---
    if (selectedCandle) {
      const x = (selectedCandle.timestamp - minTime) * xScale - clampedOffsetX;

      // Vertical Line
      ctx.save();
      ctx.strokeStyle = 'white';
      ctx.lineWidth = 1; // Linea fina blanca
      ctx.setLineDash([5, 5]); // Punteada opcional, pero usuario pidió linea blanca, mejor sólida o punteda fina? "linea vertical blanca" -> sólida suele ser mejor para marcar, pero punteada distrae menos. Probemos sólida fina.
      ctx.setLineDash([]);
      ctx.beginPath();
      ctx.moveTo(x, 0);
      ...
      // Tooltip
      const timeStr = new Date(selectedCandle.timestamp).toLocaleTimeString('es-ES', {
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit'
      });
      ...
      // Evitar que el tooltip se salga del borde derecho
      let tooltipX = x + 10;
      if (tooltipX + boxWidth > dimensions.width) {
        tooltipX = x - 10 - boxWidth;
      }

      // Posición Y fija en la parte superior para no tapar velas, o cerca del mouse?
      // El usuario dijo "sin que el tooltip tape nada". Arriba del todo suele ser seguro.
      const tooltipY = 30;
  ...
  }, [candles, currentCandle, dimensions, isInitialized, viewState, bets, selectedCandle])
  ...
  const handleDoubleClick = (e: MouseEvent) => {
    if (!lastRenderMeta.current || !canvasRef.current) return;
    const { minTime, xScale, clampedOffsetX } = lastRenderMeta.current;

    // Calcular timestamp del click
    const rect = canvasRef.current.getBoundingClientRect();
    const mouseX = e.clientX - rect.left;
    const clickedTimestamp = ((mouseX + clampedOffsetX) / xScale) + minTime;

    // Buscar vela más cercana (incluyendo reales y current)
    const allCandles = [...candles];
    if (currentCandle) allCandles.push(currentCandle);
    if (allCandles.length === 0) return;
    ...
    const closest = allCandles.reduce((prev, curr) =>
      Math.abs(curr.timestamp - clickedTimestamp) < Math.abs(prev.timestamp - clickedTimestamp) ? curr : prev
    );

    setSelectedCandle({ timestamp: closest.timestamp, price: closest.close });
  };
  ...
  }, [canvasRef, handleMouseDown, handleMouseMove, handleMouseUp, handleWheel, handleTouchStart, handleTouchMove, handleTouchEnd, handleDoubleClick]);

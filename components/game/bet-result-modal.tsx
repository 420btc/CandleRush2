...
export default function BetResultModal({ open, onOpenChange, result }: BetResultModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`text-center max-w-lg p-8 rounded-2xl border-4 shadow-2xl ${won ? 'border-yellow-400 bg-black' : 'border-yellow-400 bg-black'}`}>
        <DialogHeader>
          <DialogTitle className={`text-3xl font-extrabold mb-1 flex items-center justify-center gap-2 ${won ? "text-green-400" : "text-red-400"}`}
        ...
        <div className="grid grid-cols-1 gap-4 mt-4">
          <div className={`rounded-xl p-4 border text-lg font-bold ${won ? 'border-yellow-400 bg-black' : 'border-yellow-400 bg-black'}`}
            style={{color: won ? '#FFD600' : '#FFD600'}}>
            Ganancia: <span className={`font-mono text-2xl ml-2 ${won ? 'text-green-400' : 'text-red-400'}`}>{amount > 0 ? `+${amount.toFixed(2)}` : amount.toFixed(2)} $</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 w-full mt-6 text-base">
            <div className="rounded-xl p-4 min-w-[90px] border-2 border-yellow-400 bg-black flex flex-col items-center">
              ...
              <div className="font-mono text-xl text-white break-words">{candle.open.toFixed(2)}</div>
            </div>
            <div className="rounded-xl p-4 min-w-[90px] border-2 border-yellow-400 bg-black flex flex-col items-center">
